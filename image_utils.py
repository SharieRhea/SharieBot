...
# TODO: have some type of observer to watch for file changes so that when the theme updates
# this updates and automatically changes the colors without having to wait for an event
def get_colors() -> Tuple[str, str, str]:
    text_color, _ = get_highlight_colors("Label")
    border_color, background_color = get_highlight_colors("TelescopeBorder")
    return (text_color, border_color, background_color)


# return the foreground and background colors for a highlight group
def get_highlight_colors(highlight_name: str) -> Tuple[str, str]:
    nest_asyncio.apply()
    nvim = pynvim.attach(
    ...
    nvim.close()
    return (hex(foreground)[2:], hex(background)[2:])


def create_panel(text: str, colors: Tuple[str, str, str], height: int) -> Panel:
    return Panel(
        Align.center(
            Text(text, style=f"bold #{colors[0]} on #{colors[2]}"), vertical="middle"
        ),
...
def save_png(panel: Panel, title: str, filename: str, width: int):
    console = Console(record=True, width=width)
    console.print(panel)
    svg = console.export_svg(
        title=title, code_format=CODE_FORMAT, font_aspect_ratio=0.60
    )
    svg2png(bytestring=svg, write_to=f"resources/{filename}")


def update_image(before: str, after: str, title: str) -> None:
    text = f"{before}: {after}"
    colors = get_colors()
    save_png(
        panel=create_panel(text, colors, 3),
        title=title,
...
def update_notification_image(text: str, title: str) -> None:
    colors = get_colors()
    save_png(
        panel=create_panel(text, colors, 3),
        title=title,
